...
void main() {
  group('decodeStreamWrite', () {
    test('decodes base64 payloads, including non-ASCII', () {
      expect(VmServiceOutput.decodeStreamWrite(_write('hello\n')), 'hello\n');
      expect(
        VmServiceOutput.decodeStreamWrite(_write('héllo ✓ 日\n')),
        'héllo ✓ 日\n',
      );
    });
    ...
    test('survives a multi-byte sequence split by the VM mid-character', () {
      ...
      final first = VmServiceOutput.decodeStreamWrite({
        'bytes': base64Encode(bytes.sublist(0, 2)),
      });
    ...
    test('returns null when there is nothing to print', () {
      expect(VmServiceOutput.decodeStreamWrite({'bytes': ''}), isNull);
      expect(VmServiceOutput.decodeStreamWrite(const {}), isNull);
      expect(
        VmServiceOutput.decodeStreamWrite({'bytes': 'not base64!!'}),
        isNull,
      );
    });
  });
  ...
  group('formatLogRecord', () {
    test('prefixes with the logger name', () {
      expect(
        VmServiceOutput.formatLogRecord(
          _log(logger: 'auth', message: 'signed in'),
        ),
        '[auth] signed in\n',
      );
    });
    ...
    test('falls back to [log] when the logger is unnamed', () {
      expect(
        VmServiceOutput.formatLogRecord(_log(message: 'plain')),
        '[log] plain\n',
      );
      expect(
        VmServiceOutput.formatLogRecord(_log(logger: '', message: 'plain')),
        '[log] plain\n',
      );
    });
    ...
    test('includes error and stack trace, each prefixed', () {
      expect(
        VmServiceOutput.formatLogRecord(
          _log(logger: 'net', message: 'failed', error: 'boom', stack: 'at x'),
        ),
        '[net] failed\n[net] boom\n[net] at x\n',
      );
    });
    ...
    test('returns null when the record carries no text', () {
      expect(VmServiceOutput.formatLogRecord(_log(logger: 'empty')), isNull);
      expect(
        VmServiceOutput.formatLogRecord(const {'streamId': 'Logging'}),
        isNull,
      );
